...
from cfdi.v40.pdf import catalogos
...
def generate_pdf(
    ...
    *,
    ...
    pdf.drawRightString(
        x_offset,
        y_offset,
        f'Tipo de Comprobante: {cfdi.tipo_de_comprobante} - {catalogos.get_tipo_de_comprobante(cfdi.tipo_de_comprobante)}',
    )
    ...
    pdf.drawString(
        x_offset,
        y_offset,
        f'{cfdi.emisor.regimen_fiscal} - {catalogos.get_regimen_fiscal(cfdi.emisor.regimen_fiscal)}',
    )
    ...
    pdf.drawString(
        x_offset,
        y_offset,
        f'USO CFDI: {cfdi.receptor.uso_cfdi} - {catalogos.get_uso_cfdi(cfdi.receptor.uso_cfdi)}',
    )
    ...
    pdf.drawString(
        x_offset,
        y_offset,
        f'Régimen fiscal: {cfdi.receptor.regimen_fiscal_receptor} - {catalogos.get_regimen_fiscal(cfdi.receptor.regimen_fiscal_receptor)}',
    )
    ...
    for concepto in cfdi.conceptos:
        ...
        # unidad
        unidad = Paragraph(
            f'{concepto.clave_unidad} - {catalogos.get_clave_unidad(concepto.clave_unidad)}',
            concepts_table_body_center_style,
        )
        ...
        descripcion = [
            Paragraph(
                f'{concepto.clave_prod_serv} - {catalogos.get_clave_prod_serv(concepto.clave_prod_serv)}'.upper(),
                concepts_table_body_left_black_style,
            ),
            Paragraph(
        ...
        impuestos = [
            Paragraph(
                f'{traslado.impuesto} - {catalogos.get_impuesto(traslado.impuesto)} {format_as_percentage(traslado.tasa_o_cuota)}%',
                concepts_table_body_right_style,
            ),
            Paragraph(
    ...
    metodo_de_pago = Paragraph(
        f'Método de Pago: {cfdi.metodo_pago} - {catalogos.get_metodo_pago(cfdi.metodo_pago)}',
        concepts_table_body_center_style,
    )
    ...
    forma_de_pago = Paragraph(
        f'Forma de Pago: {cfdi.forma_pago} - {catalogos.get_forma_pago(cfdi.forma_pago)}',
        concepts_table_body_center_style,
    )
